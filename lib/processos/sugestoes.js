...
//
// AGENT_TEMPLATES é passado mesmo os três processos autorados pelo redator
// não precisando dele diretamente (a função deles já captura os templates
// por conta própria, ver criarRenderizador em catalogo.js) — mas
// situacao_pagamento é 100% declarativo e depende deste parâmetro para
// encontrar seu próprio template.
async function calcularSugestoes(situacao, { redator, mensagens } = {}) {
  ...
  const resultados = await Promise.all(
    CATALOGO_PROCESSOS.map((processo) => executarProcesso(processo, contexto, ferramentas, AGENT_TEMPLATES, apoio))
  );
