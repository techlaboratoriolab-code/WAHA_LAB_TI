...
// mensagem baseada numa Resposta Rápida de referência e no contexto real da
// conversa; só cai no template fixo (o mecanismo original, ainda testado e
// funcional) se o redator não estiver configurado ou falhar. Isso mantém a
// garantia de sempre haver uma sugestão quando o dado existe, com ou sem LLM.
...
function criarRenderizador({ templateId, slots, referenciaId }) {
  const referencia = REFERENCIAS_RESPOSTA_RAPIDA[referenciaId];
  if (!referencia) {
    // Falha alto e no carregamento do módulo — um id de referência errado
    // não pode só quebrar silenciosamente na primeira consulta real.
    throw new Error(`Referência de Resposta Rápida "${referenciaId}" não existe (processo "${templateId}").`);
  }
  ...
  return async function renderizar(situacao, contexto, apoio) {
    if (apoio && typeof apoio.redigir === 'function') {
      ...
      if (redigido && redigido.texto) {
        return { templateId: `${templateId}:llm`, texto: redigido.texto, baseadoEm: referencia.origemId };
      }
    }
...
const CATALOGO_PROCESSOS = [
  {
    ...
    renderizar: criarRenderizador({
      ...
      referenciaId: 'biologiamolecular'
    })
  },
  {
    id: 'status_exame',
